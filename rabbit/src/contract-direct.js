...
var vue = new Vue({
  ...
  methods: {
    init_job (job) {
      this.$set('job', job)
      this.$set('contract.name', job.name)
      this.$set('pay_type', job.paymethod)
      this.$set('job_id', job.id)
      this.$set('amount', job.budget)
    },
    init_freelancer (freelancer) {
      this.$set('freelancer', freelancer)
      this.$set('avatar', freelancer.avatar)
      this.$set('name', freelancer.name)
    },
    show_v () {
    ...
    buildCommon () {
      let offer = {}
      offer.user_id = this.freelancer.id
      offer.job_id = this.job.id
...
$(function () {
  let init = function () {
    let job_id = location.pathname.split('/')[3]
    let uuid = location.pathname.split('/')[4]
    vue.$set('job_id', job_id)
    vue.$set('uuid', uuid)

    initData(job_id, uuid)
  }

  let initData = function (job_id, uuid) {
    $.getJSON('/api/jobs', {job_id: vue.$data.job_id}, function (data) {
      if (data.error_code === 0) {
        vue.init_job(data.job)
      }
    })
    $.getJSON('/api/user/profile', {uuid: vue.$data.uuid}, function (data) {
      if (data.error_code === 0) {
        vue.init_freelancer(data.profile)
      }
    })
  }
